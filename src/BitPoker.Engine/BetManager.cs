...
namespace BitPoker.Engine
{
    ...
    public class BetManager : IBetManager
    {
        ...
        private Dictionary<string, UInt64> committedTotal;
        private Dictionary<string, UInt64> committedThisRound;
        private Dictionary<string, UInt64> startingChips;
        ...
        public BetManager(Dictionary<string,UInt64> namesToChips, BettingStructure structure, UInt64[] blinds, UInt64 ante )
        {
            ...
            this.smallBlind = blinds.Length > 1 ? blinds[0] : 0;
            ...
            committedThisRound = new Dictionary<string, UInt64>();
            committedTotal = new Dictionary<string, UInt64>();
            startingChips = new Dictionary<string, UInt64>();
        ...
        private Action validatePostAnte(Action action)
        {
            ...
            action.Amount = Math.Min(startingChips[action.PublicKey], ante);

			if (action.Amount + committedTotal[action.PublicKey] >= startingChips[action.PublicKey])
        ...
        private Action validatePostSmallBlind(Action action)
        {
            ...
            action.Amount = Math.Min(startingChips[action.PublicKey], smallBlind);
        ...
        private Action validatePostBigBlind(Action action)
        {
            ...
            action.Amount = Math.Min(startingChips[action.PublicKey], bigBlind);
        ...
        private Action validateFold(Action action)
        {
            ...
            if (committedTotal[action.PublicKey] >= mostCommitted)
        ...
        {
            if (committedTotal[action.PublicKey] < mostCommitted)//fold if you can't check
                return validateFold(action);
        ...
        {
            if (committedTotal[action.PublicKey] >= mostCommitted)//check if you don't have to call anything
                return validateCheck(action);

            action.Amount = mostCommitted - committedTotal[action.PublicKey];
        ...
        private Action validateAllIn(Action action)
        {
            ...
            action.Amount = startingChips[action.PublicKey] - committedTotal[action.PublicKey];
            return action;
        }
        ...
        private Action validateBet(Action action)
        {
            ...
            UInt64 amt = Math.Max(action.Amount, MinimumRaise);

            //if the bet is larger than the player's remaining chips, change him to all-in.
            if (amt + mostCommitted >= startingChips[action.PublicKey])
            {
                //action.ActionType = Action.ActionTypes.AllIn;
                action.AllIn = true;
                action.Amount = startingChips[action.PublicKey] - committedTotal[action.PublicKey];
        ...
        private Action validateRaise(Action action)
        {
            ...
            UInt64 amt = Math.Max(action.Amount, mostCommitted - committedTotal[action.PublicKey] + minimumRaise);

            //if the bet is larger than the player's remaining chips, change him to all-in.
            if (amt + committedTotal[action.PublicKey] >= startingChips[action.PublicKey])
            {
                //action.ActionType = Action.ActionTypes.AllIn;
                action.AllIn = true;
                action.Amount = startingChips[action.PublicKey] - committedTotal[action.PublicKey];
        ...
        {
            UInt64 amt = action.Amount;
            string name = action.PublicKey;
            ...
            if (action.AllIn)
            {
                ...
                UInt64 total = committedTotal[name];
