...
namespace BitPoker.Engine
{
    ...
    public class PotManager : IEnumerable<Pot>, IPotManager
    {
        ...
        /// </summary>
        /// <param name="playerIdx">Name of the betting player</param>
        /// <param name="amt">Amount the player is betting</param>
        private void AddBet(int playerIdx, UInt64 amt)
        {
            players[playerIdx].committed += amt;
            tempPot += amt;
            this.Total += amt;
        ...
        /// </summary>
        /// <param name="playerIdx">Name of the folding player</param>
        private void AddFold(int playerIdx)
        {
            players[playerIdx].stillIn = false;
            foreach (Pot pot in pots)
            {
                pot.RemovePlayer(playerIdx);
            }
        }
        #endregion
        ...
        public void UpdatePots()
        {
            ...
            UInt64[] tempCom = new UInt64[players.Length];
            ...
            while (smallest >= 0)
            {
                ...
                UInt64 toSubtract = tempCom[smallest];
                ...
                for (int i = 0; i < players.Length; i++)
                {
                    if (tempCom[i] > 0)
                    {
                        ...
                        //put in.
                        UInt64 amt = Math.Min(toSubtract,tempCom[i]);
                        if (tempStill[i])
                        {
        ...
        public IList<Winner> GetWinners(uint[] handStrengths)
        {
            List<Winner> winners = new List<Winner>();
            foreach (Pot pot in pots)
            {
                pot.GetWinners(handStrengths, winners);
            }
            return winners;
        }
